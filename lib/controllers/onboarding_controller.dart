    ...
    if (name.text.isNotEmpty &&
      ...
      if (getPhone.text.length < 10) {
        ...
        await _functions
          ...
          // Navigate to the main screen after form submission
          Get.offAllNamed('/survey-intro-screen');
        }).catchError((error) {
          print(error);
          Get.snackbar("Error", "Failed to submit form. Please try again.");
        }).whenComplete(() => Indicator.closeLoading());
      }
    } else {
  ...
  void onSubmitAnonymousName() async {
    if (anonymousName.text.isNotEmpty) {
      ...
      await _functions.storeAnonymousName(anonymousName.text).then((value) {
        anonymousName.clear();
        FocusManager.instance.primaryFocus?.unfocus();
        // Navigate to the main screen after storing the anonymous name
        Get.offAllNamed('/survey-intro-screen');
      }).catchError((error) {
        print(error);
        Get.snackbar("Error", "Failed to store anonymous name. Please try again.");
      }).whenComplete(() => Indicator.closeLoading());
    } else {
